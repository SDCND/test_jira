...
F = np.array( [1, 0, dt, 0,0, 1, 0, dt,0, 0, 1, 0,0, 0, 0, 1 ]).reshape(4,4)
...
while(True):
    ...
    if(camshift):
        ...
        #cv.imshow("Backprojection", cv.resize(aa,(400,250)))
...
cv.imshow('ColorMask',colorMask).imshow('ColorMask',cv.resize(colorMask,(800,600)))
cv.imshow('mask', bgs)
